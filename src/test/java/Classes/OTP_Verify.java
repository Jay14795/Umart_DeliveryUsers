...
public class OTP_Verify {
    public static void OTP(AppiumDriver<MobileElement> driver) {
        ...
        final var finger6 = new PointerInput(PointerInput.Kind.TOUCH, "finger6");
        tapPoint = new Point(285, 1198);
        tap = new Sequence(finger6, 1);
        tap.addAction(finger6.createPointerMove(Duration.ofMillis(0),
                PointerInput.Origin.viewport(), tapPoint.x, tapPoint.y));
        tap.addAction(finger6.createPointerDown(PointerInput.MouseButton.LEFT.asArg()));
        tap.addAction(new PointerInput(PointerInput.Kind.TOUCH, "finger6").createPointerUp(PointerInput.MouseButton.LEFT.asArg()));
        driver.perform(Arrays.asList(tap));

        final var finger7 = new PointerInput(PointerInput.Kind.TOUCH, "finger7");
        tapPoint = new Point(443, 1192);
        tap = new Sequence(finger7, 1);
        tap.addAction(finger7.createPointerMove(Duration.ofMillis(0),
                PointerInput.Origin.viewport(), tapPoint.x, tapPoint.y));
        tap.addAction(finger7.createPointerDown(PointerInput.MouseButton.LEFT.asArg()));
